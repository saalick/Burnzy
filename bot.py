...
# Set up logging
logging.basicConfig(level=logging.DEBUG, filename='burn_bot.log', filemode='w', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def load_last_burn_time():
    if os.path.exists(last_burn_file):
        with open(last_burn_file, 'r') as f:
            data = json.load(f)
            last_burn_time_str = data.get('last_burn_time')
            return datetime.fromisoformat(last_burn_time_str) if last_burn_time_str else None
    else:
        return None

# Function to save the last burn transaction time to the JSON file
def save_last_burn_time(last_burn_time):
    data = {'last_burn_time': last_burn_time.isoformat()}
...
def send_message(message):
    bot.send_message(chat_id='-1002118601081', text=message, parse_mode="HTML", disable_web_page_preview=True)  # Replace with your group chat ID
...
def send_video_message(video_url, message):
    bot.send_video(chat_id='-1002118601081', video=video_url, caption=message, parse_mode="HTML")

RPC_URL = 'https://base.publicnode.com'
CONTRACT_ADDRESS = '0xa2Eb776f262A7D001Df8606F74fcFD3Ee4A31cc4'
TO_ADDRESS = '0x000000000000000000000000000000000000dEaD'  # Adjust the to address
...
def send_transaction():
    ...
    while True:
        try:
            ...
            if last_burn_time is None or (datetime.now() - last_burn_time) >= timedelta(hours=12):
                token_amount = Web3.toWei(amount_tokens, 'gwei')
                ...
                save_last_burn_time(last_burn_tx_time)
...
🐦<a href='https://x.com/thisisbased_'>Twitter</a> 💬<a href='https://t.me/ThisIsBasedFOB'>Telegram</a> 🌐<a href='https://fineonbase.wtf/'>Website</a>
                ...
                time_since_last_burn = datetime.now() - last_burn_time
